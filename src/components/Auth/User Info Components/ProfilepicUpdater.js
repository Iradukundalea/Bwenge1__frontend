...
export const ProfilepicUpdater = () => {
  const { id } = useParams();
  ...
  const [updaterLoader, setUpdaterLoader] = useState(false);
  ...
  const generateDownload = (canvas, crop) => {
    ...
    canvas.toBlob(
      (blob) => {
        ...
        const url = `${thekomp}/auth/updateprofilepic/${id}`;
        const config = {
          method: "patch",
          url: url,
          Headers: {
