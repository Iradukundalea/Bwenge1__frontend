import React from "react";
import renderHTML from "react-render-html";
import "./../styles/QuizTime.css";
import { CKEditor } from "@ckeditor/ckeditor5-react";
import ClassicEditor from "@ckeditor/ckeditor5-build-classic";
import DocsViewer from "../../../../DocsViewer";
import thekomp from "./../../../../../thekomp";
import { IoTrashBin } from "react-icons/io5";

const QuizProblem = ({ selectedProb, setSelectedProb, setUserAnswerHandler, updateUserAnswerFiles }) => {
  const API = thekomp;

  const renderAnswers = () => {
    switch (selectedProb.problem.type) {
      case "True or False":
        return (
          <div>
            <div className="answerItemMC" style={{ display: "grid", gridTemplateColumns: "5% 95%" }}>
              <div>
                <input
                  onClick={(e) => {
                    setUserAnswerHandler("true");
                  }}
                  type="radio"
                  id="html"
                  checked={selectedProb.Useranswer === "true"}
                  name="fav_language"
                  value="HTML"
                  style={{ width: "20px", height: "20px" }}
                />
              </div>
              <div>True</div>
            </div>
            <div className="answerItemMC" style={{ display: "grid", gridTemplateColumns: "5% 95%" }}>
              <div>
                <input
                  onClick={(e) => {
                    setUserAnswerHandler("false");
                  }}
                  type="radio"
                  checked={selectedProb.Useranswer === "false"}
                  id="html"
                  name="fav_language"
                  value="HTML"
                  style={{ width: "20px", height: "20px" }}
                />
              </div>
              <div>False</div>
            </div>
          </div>
        );
      case "Short Answer":
        return (
          <div className="ui input">
            <input
              type="text"
              value={selectedProb.Useranswer}
              onChange={(e) => {
                setUserAnswerHandler(e.target.value);
              }}
              placeholder="Answer..."
            />
          </div>
        );
      case "Multiple Choice":
        return selectedProb.problem.answers.map((answer) => {
          return (
            <div
              onClick={(e) => {
                setUserAnswerHandler(answer);
              }}
              className="answerItemMC"
              style={{ display: "grid", gridTemplateColumns: "5% 95%" }}
            >
              <div style={{ paddingTop: "5px" }}>
                <input
                  checked={answer === selectedProb.Useranswer}
                  type="radio"
                  id="html"
                  name="fav_language"
                  value="HTML"
                  style={{ width: "20px", height: "20px" }}
                />
              </div>
              <div>{renderHTML(answer)}</div>
            </div>
          );
        });
      case "Long Answer":
        return (
          <div>
            <CKEditor
              editor={ClassicEditor}
              data={selectedProb.Useranswer}
              onChange={(event, editor) => {
                const data = editor.getData();
                setUserAnswerHandler(data);
              }}
            />
          </div>
        );
    }
  };
  const renderQuestionFile = () => {
    const extenso = selectedProb.problem.questionFile.substr(selectedProb.problem.questionFile.length - 3, 3);
    if (extenso === "pdf") {
      return (
        <div>
          <button class="ui right labeled icon button my-3">
            <a
              href={`${API}/${selectedProb.problem.questionFile}`}
              target="_blank"
              style={{
                opacity: "0.0",
                position: "absolute",
                " top": " 0",
                left: " 0",
                bottom: " 0",
                right: " 0",
                " width": "100%",
                height: "100%",
                cursor: "pointer",
              }}
            />
            <i class="file alternate icon"></i>
            There is a Question File,Download it
          </button>
        </div>
      );
    } else {
      return <img style={{ maxHeight: "40vh" }} className="img-fluid" src={`${thekomp}/${selectedProb.problem.questionFile}`} />;
    }
  };
  const renderAnswerFiles = () => {
    return selectedProb.studentTeacherFiles.map((item) => {
      if (item.fileloc.type) {
        if (item.fileloc.type == "application/pdf") {
          return (
            <div style={{ width: "40vw" }} className="row">
              <div className="col-10">
                <iFrame class="embed-responsive-item" width="100%" height="100%" src={URL.createObjectURL(item.fileloc)} />
              </div>

              <div className="col-2">
                <IoTrashBin
                  onClick={(e) => {
                    let proba = selectedProb.studentTeacherFiles;
                    proba = proba.filter((pop) => pop != item);
                    setSelectedProb({ ...selectedProb, studentTeacherFiles: proba });
                    updateUserAnswerFiles();
                  }}
                  style={{
                    width: "30px",
                    height: "30px",
                    cursor: "pointer",
                  }}
                />
              </div>
            </div>
          );
        } else if (item.fileloc.type == "image/jpeg" || item.fileloc.type == "image/png") {
          return (
            <div style={{ width: "40vw" }} className="row">
              <div className="col-10">
                <img className="img-fluid w-4 h-2" style={{ border: "2px solid black" }} src={URL.createObjectURL(item.fileloc)} />
              </div>

              <div className="col-2">
                <IoTrashBin
                  onClick={(e) => {
                    let proba = selectedProb.studentTeacherFiles;
                    proba = proba.filter((pop) => pop != item);
                    console.log(proba);
                    if (!proba) {
                      proba = [];
                    }
                    setSelectedProb({ ...selectedProb, studentTeacherFiles: proba });
                    updateUserAnswerFiles();
                  }}
                  style={{
                    width: "30px",
                    height: "30px",
                    cursor: "pointer",
                  }}
                />
              </div>
            </div>
          );
        }
      } else {
        if (item.fileloc.substr(item.fileloc.length - 3, 3) == "jpg" || item.fileloc.substr(item.fileloc.length - 3, 3) == "png") {
          return (
            <div style={{ width: "40vw" }} className="row">
              <div className="col-10">
                <img className="img-fluid w-4 h-2" src={`${API}/${item.fileloc}`} />
              </div>

              <div className="col-2">
                <IoTrashBin
                  onClick={(e) => {
                    let proba = selectedProb.studentTeacherFiles;
                    proba = proba.filter((pop) => pop != item);
                    setSelectedProb({ ...selectedProb, studentTeacherFiles: proba });
                    updateUserAnswerFiles();
                  }}
                  style={{
                    width: "30px",
                    height: "30px",
                    cursor: "pointer",
                  }}
                />
              </div>
            </div>
          );
        } else if (item.fileloc.substr(item.fileloc.length - 3, 3) == "pdf") {
          return (
            <div style={{ width: "40vw" }} className="row">
              <div className="col-10">
                <iFrame class="embed-responsive-item" width="100%" height="100%" src={`${API}/${item.fileloc}`} />
              </div>

              <div className="col-2">
                <IoTrashBin
                  onClick={(e) => {
                    let proba = selectedProb.studentTeacherFiles;
                    proba = proba.filter((pop) => pop != item);
                    setSelectedProb({ ...selectedProb, studentTeacherFiles: proba });
                    updateUserAnswerFiles();
                  }}
                  style={{
                    width: "30px",
                    height: "30px",
                    cursor: "pointer",
                  }}
                />
              </div>
            </div>
          );
        }
      }
    });
  };

  return (
    <div>
      <div className="problemStatement">{renderHTML(selectedProb.problem.questionQ)}</div>
      <div style={{ maxHeight: "400px" }}>{selectedProb.problem.questionFile && renderQuestionFile()}</div>
      <div className="answersDescription ui raised segment">{renderAnswers()}</div>
      <div>
        {selectedProb.problem.fileRequired && (
          <div>
            <button class="ui right labeled icon button my-3">
              <input
                type="file"
                id="answerfile1"
                onChange={(e) => {
                  var fileInput = document.getElementById("answerfile1");
                  var filePath = fileInput.value;
                  // Allowing file type
                  var allowedExtensions = /(\.jpg|\.pdf|\.png|\.doc|\.txt)$/i;

                  if (!allowedExtensions.exec(filePath)) {
                    alert("File type not allowed");
                    return false;
                  } else {
                    console.log(selectedProb);
                    const proba = selectedProb.studentTeacherFiles;
                    console.log(proba);
                    if (proba) {
                      proba.push({ fileloc: document.getElementById("answerfile1").files[0], sender: "Me", timeSent: new Date() });
                      setSelectedProb({ ...selectedProb, studentTeacherFiles: proba });
                      updateUserAnswerFiles();
                    } else {
                      var studentTeacherFiles = [];
                      studentTeacherFiles.push({
                        fileloc: document.getElementById("answerfile1").files[0],
                        sender: "Me",
                        timeSent: new Date(),
                      });
                      setSelectedProb({ ...selectedProb, studentTeacherFiles });
                      updateUserAnswerFiles();
                      // var selprob = selectedProb;
                      // selprob.studentTeacherFiles = [];
                      // selprob.studentTeacherFiles.push({
                      //   fileloc: document.getElementById("answerfile1").files[0],
                      //   sender: "Me",
                      //   timeSent: new Date(),
                      // });
                      // setSelectedProb(selprob);
                      // updateUserAnswerFiles();
                    }
                  }
                }}
                style={{
                  opacity: "0.0",
                  position: "absolute",
                  " top": " 0",
                  left: " 0",
                  bottom: " 0",
                  right: " 0",
                  " width": "100%",
                  height: "100%",
                  cursor: "pointer",
                }}
              />
              <i class="file alternate icon"></i>
              Add Answer File
            </button>
            .jpg .png .pdf .txt .doc
          </div>
        )}
      </div>
      <div>{selectedProb.studentTeacherFiles && renderAnswerFiles()}</div>
    </div>
  );
};

export default QuizProblem;
