...
const AssignmentPackage = () => {
  ...
  const { data, loading, error } = useIndivMoocAssignment(courseId, assignmentId);
  ...
  const quiz = data.getMoocAssignment;
    ...
    switch (quizPackage) {
      ...
      case "QuizTime":
        if (unfinishedQuizData) {
          if (unfinishedQuizData.assignment) return <QuizTime2 unfinishedQuizData={unfinishedQuizData} quiz={quiz} />;
        }
        return <QuizTime quiz={quiz} />;
    }
  };
