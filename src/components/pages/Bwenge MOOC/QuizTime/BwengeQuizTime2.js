...
const BwengeQuizTime2 = ({ quiz, unfinishedQuizData }) => {
  const API = thekomp;
  ...
  const thotime = unfinishedQuizData.QuizDuration;
  ...
  useEffect(() => {
    ...
    let url = `${API}/quizTime/updatebwengeuserquizinfo/${localStorage.getItem("userId")}/${courseId}`;
    const config = {
      method: "patch",
      url: url,
      headers: {
  ...
  const postUserResults = (score) => {
    ...
    for (var i = 0; i < userReponses.length; i++) {
      for (var j = 0; userReponses[i].studentTeacherFiles && j < userReponses[i].studentTeacherFiles.length; j++) {
        if (userReponses[i].studentTeacherFiles[j].fileloc && userReponses[i].studentTeacherFiles[j].fileloc.type) {
          ...
          userReponses[i].studentTeacherFiles[j].fileloc = `Bwengelongcourses\\${localStorage.getItem(
            "courseTitle"
          )}\\Students\\${localStorage.getItem("userId")}\\exams\\${quiz.title}\\${userReponses[i].studentTeacherFiles[j].fileloc.name}`;
        }
      }
    }
    ...
    if (quiz.title.substr(quiz.title.length - 6, 4) === "Exam") {
      url = `${API}/enroll/postbwengeexamresult/${localStorage.getItem("userId")}/${courseId}`;
    } else if (quiz.title.substr(quiz.title.length - 6, 4) === "Quiz") {
      url = `${API}/enroll/postbwengequizresult/${localStorage.getItem("userId")}/${courseId}`;
    } else {
      url = `${API}/enroll/postbwengeassignmentresult/${localStorage.getItem("userId")}/${courseId}`;
    }
    const config = {
      method: "patch",
      url: url,
...
export default BwengeQuizTime2;
