...
import { useIndivMoocQuiz } from "../hooks/useIndivMooc";
import { useUserQuizData } from "./quiz hooks/useUserQuizInfo";
import QuizTime2 from "./QuizTime2";
import thekomp from "./../../../../thekomp";
import { useBwengeCourseQuiz } from "./quiz hooks/useBwengelongCourses";
import BwengeQuizTime2 from "./BwengeQuizTime2";
import BwengeQuizTime from "./BwengeQuizTime";

const BwengeQuizPackage = () => {
  //   const [BwengequizPackage, setBwengeQuizPackage] = useState("instructions");
  const API = thekomp;
  ...
  let BwengequizPackage = "instructions";
  ...
  const { data, loading, error } = useBwengeCourseQuiz(courseId, assignmentId);
  ...
  if (unfinishedQuizData) BwengequizPackage = "QuizTime";
  ...
  const onStartquiz = () => {
    ...
    BwengequizPackage = "QuizTime";
    window.location.reload(false);
  };

  const renderBwengeQuizPackage = () => {
    switch (BwengequizPackage) {
      case "instructions":
        return (
          <div>
            <button
  ...
  return <div className="quizTimeBody">{renderBwengeQuizPackage()}</div>;
};

export default BwengeQuizPackage;
