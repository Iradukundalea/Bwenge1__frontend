...
import { useBwengeCourseDescription } from "./hooks.js/uselongcourses";
...
const EnrollCoursePage = () => {
  ...
  const { data, loading, error } = useBwengeCourseDescription(id);
  ...
  const course = data.getLongCourse;
  ...
  const enrollUser = () => {
    if (!localStorage.getItem("authToken")) {
      // history.push("/login");
      alert("Please, Login first to access Bwenge Long courses!!");
      return;
    }
  ...
  const renderChapters = course.chapters.map((chap, index) => {
    ...
    return (
      <div key={chap.title}>
        <div className={`title ${active}`} onClick={() => onChapterClick(index)} style={{ fontSize: "1.5rem" }}>
  ...
  return (
    <div className="longCourseBody ui segment">
      <div className="row mx-5 my-3 border border-secondary rounded ">
        ...
        <div className=" col-md-9 bg-primary bg-gradient">
          <div className="title mt-2">{course.title}</div>
          <div className="headline mt-2">{course.lead_headline}</div>
          <div className="instructors mt-2">
            Instructed By
          ...
          <div>
            <span className="lastUpdate">Last updated: {course.lastUpdated.substr(0, 10)}</span>
          </div>

          <button onClick={(e) => enrollUser()} class="ui right compact icon black button">
      ...
      <div className="ui segment">
        <div class="ui borderless menu" id="navbar" ref={navRef}>
          <a href="#desc" class={selectedMenu === "description" ? "item active" : "item"} onClick={(e) => setLectedMenu("description")}>
            Description
          </a>
          <a href="#obj" class={selectedMenu === "objectives" ? "item active" : "item"} onClick={(e) => setLectedMenu("objectives")}>
            Objectives
          </a>
          <a href="#preview" class={selectedMenu === "preview" ? "item active" : "item"} onClick={(e) => setLectedMenu("preview")}>
            Preview
          </a>
          <a href="#curric" class={selectedMenu === "curriculum" ? "item active" : "item"} onClick={(e) => setLectedMenu("curriculum")}>
            Curriculum
          </a>
          <a class={selectedMenu === "reviews" ? "item active" : "item"} onClick={(e) => setLectedMenu("reviews")}>
            Reviews
          </a>
        </div>
