...
const EditPaper = () => {
    ...
    const submitPaperHandler = async (e) => {
        ...
        formData.append('selectedFile', document.getElementById("whyLong").files[0])
        const config = {
            method: "patch",
            url: `http://localhost:5000/paper/editpaper/${selectedPaper._id}`,
            headers: {
    ...
    return (
        <div className="createpaperPage">
            <div className="center">
                <h1> Edit Paper</h1>
                <form onSubmit={submitPaperHandler} method="post" encType="multipart/formdata">
                    ...
                    <div>
                        ...
                        <DatePicker className="cal" selected={paper.DateOfPublication} onChange={(date) => setPaper({ ...paper, DateOfPublication: date })} />
                    ...
                    <div class="inputbox">
                        <input onClick={(e) => submitPaperHandler(e)} type="button" value="submit" />
                    </div>
                    {paper.errors && <span className="error-message">{paper.errors}</span>}
                </form>
