...
import { useGetUnivProjects } from "./Hooks/useUnivProjects";
const UniversityProjects = () => {
  ...
  const getPaper = (paper) => {
    ...
    if (!localStorage.getItem("authToken")) {
      ...
      console.log("ok ok");
      history.push(`/univproject/${paper.id}`);
    }
  };
  const { loading, error, data } = useGetUnivProjects();
  if (loading) {
    return (
  ...
  const papers = data.getAllApprovedUnivprojects;
        ...
        if (selectedYear.length !== 0) {
          ...
          return (
            <tr onClick={(e) => getPaper(item)} style={{ cursor: "pointer" }}>
              ...
              <td data-label="Country">{item.university}</td>
              <td data-label="Publication Date">{item.submissionDate.substr(0, 10)}</td>
            </tr>
          );
        }
    ...
    <div className="ui container">
      ...
      <button
        onClick={(e) => {
          ...
          history.push("/createunivproject");
        }}
