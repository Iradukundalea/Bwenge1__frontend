...
const CreateCommunityArticle = () => {
  const queryParams = new URLSearchParams(window.location.search);
  const communityID = queryParams.get("cid");
  ...
  const onsubmitArticleHnadler = async (e) => {
    ...
    formData.append("field", localStorage.getItem("cField"));
    formData.append("department", localStorage.getItem("cDepart"));
    formData.append("communityConnected", communityID);
    ...
    } else url = `${thekomp}/community/createqa`;
    const config = {
      method: "post",
      url,
      headers: {
              ...
              {question.answers.map((repons) => {
                ...
                return (
                  ...
                  >
                    <div style={{ paddingTop: "10px" }}>
                      <input
                        checked={question.correctAnswer.includes(repons)}
                        onClick={(e) => {
                          var correctAnswers = question.correctAnswer;
                          correctAnswers = [repons];
                          setQuestion({ ...question, correctAnswer: correctAnswers });
                        }}
                        type="radio"
                        id="html"
                        name="fav_language"
                    ...
                    <div>
                      <IoTrashBin
                        onClick={(e) => {
                          var reponses = question.answers;
                          var corecto = question.correctAnswer;
                          reponses = _.without(reponses, repons);
                          corecto = _.without(corecto, repons);
                          setQuestion({ ...question, answers: reponses, correctAnswer: corecto });
                        }}
                        style={{
                          width: "30px",
                          height: "30px",
                  ...
                  >
                    <div style={{ paddingTop: "10px" }}>
                      <input
                        ...
                        onClick={(e) => {
                          var correctAnswers = question.correctAnswer;
                          if (correctAnswers.includes(repons)) {
                            correctAnswers = correctAnswers.filter((itema) => itema != repons);
                            setQuestion({ ...question, correctAnswer: correctAnswers });
                          } else {
                            correctAnswers = [...correctAnswers, repons];
                            setQuestion({ ...question, correctAnswer: correctAnswers });
                          }
                        }}
                        type="checkbox"
                    ...
                    <div>
                      <IoTrashBin
                        onClick={(e) => {
                          ...
                          reponses = _.without(reponses, repons);
                          corecto = _.without(reponses, repons);
                          setQuestion({ ...question, answers: reponses, correctAnswer: corecto });
                        }}
                        style={{
                          width: "30px",
        ...
        return (
          <div className="ui raised segment">
            <div className="answerItemMC" style={{ display: "grid", gridTemplateColumns: "5% 95%" }}>
              <div>
                <input
                  checked={question.correctAnswer.includes("true")}
                  onClick={(e) => {
                    var cd = ["true"];
                    setQuestion({ ...question, correctAnswer: cd });
                  }}
                  type="radio"
                  id="html"
                  name="fav_language"
            ...
            <div className="answerItemMC" style={{ display: "grid", gridTemplateColumns: "5% 95%" }}>
              <div>
                <input
                  checked={question.correctAnswer.includes("false")}
                  onClick={(e) => {
                    var cd = ["false"];
                    setQuestion({ ...question, correctAnswer: cd });
                  }}
                      ...
                      <div className="mapItemsObj">
                        ...
                        <i
                          ...
                          onClick={(e) => {
                            ...
                            setQuestion({ ...question, correctAnswer: reponses });
                          }}
                        ></i>
                      </div>
                    );
              ...
              <div class="two fields">
                <div class="field">
                  <input
                    ...
                    placeholder="Pair One"
                    value={matchingPair.pairOne}
                    onChange={(e) => {
                      setMatchingPair({ ...matchingPair, pairOne: e.target.value, error: "" });
                    }}
                  />
                </div>
                <div class="field">
                  <input
                    ...
                    placeholder="Pair Two"
                    value={matchingPair.pairTwo}
                    onChange={(e) => {
                      setMatchingPair({ ...matchingPair, pairTwo: e.target.value, error: "" });
                    }}
                  />
                </div>
                {matchingPair.error && (
                  <div class="ui error message">
                    ...
                    <p>{matchingPair.error}</p>
                  </div>
                )}
                <button
                  className="ui primary button"
                  onClick={(e) => {
                    if (!matchingPair.pairOne) {
                      setMatchingPair({ ...matchingPair, error: "Add Pair One" });
                    } else if (!matchingPair.pairTwo) {
                      setMatchingPair({ ...matchingPair, error: "Add Pair Two" });
                    } else {
                      var pais = question.answers;
                      var pais = [...pais, matchingPair.pairOne, matchingPair.pairTwo];
                      setQuestion({ ...question, answers: pais });
                      setMatchingPair({
                        pairOne: "",
                        pairTwo: "",
                        error: "",
                      ...
                      <a class="ui teal image large label">
                        <i
                          onClick={(e) => {
                            ...
                            var inda = index;
                            answs = answs.filter((_, index) => index != inda && index != inda + 1);
                            setQuestion({ ...question, answers: answs });
                          }}
                          className="ui close icon me-2"
                        ></i>
                        {x}
                        <div class="detail">{question.answers[index + 1]}</div>
                      </a>
                    </div>
                  );
                }
          ...
          <div className="field">
            ...
            <div>
              <select value={question.type} onChange={(e) => setQuestion({ ...question, type: e.target.value })}>
                ...
                <option class="item" value="Multiple Answers">
                ...
                {/* <option class="item" value="Matching">
                  Matching Question
                </option> */}
              </select>
            </div>
  ...
  return (
    <div className="ui segment">
      ...
      <div className="ui form">
        ...
        <div className="field">
          <label>Type:</label>
          <select onClick={(e) => setArticle({ ...article, type: e.target.value })}>
            <option className="item" value="article">
              Article
            </option>
            <option className="item" value="qas">
              Ikibazo/Igitekerezo
            </option>
          </select>
        </div>
                ...
                <div className="mapItemsObj">
                  ...
                  <i
                    ...
                    onClick={(e) => {
                      ...
                      tagos = tagos.filter((item) => item !== objective);
                      setArticle({
                        ...article,
                        tags: tagos,
                      });
      ...
      {addQuestionModal.onOpen && (
        <Modal
          ...
          onDismiss={(e) => {
            ...
            setAddQuestionModal({ ...addQuestionModal, onOpen: false });
          }}
        />
      )}
